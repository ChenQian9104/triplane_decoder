...
class MultiOccupancyDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path, subset_size = 100, device = "cuda"):
        ...
        for file_path in self.file_list:
            ...
            idx = np.random.randint(0, curr_data.shape[1], 500000)
            curr_data = torch.Tensor(curr_data[idx])
...
class MultiOccupancyDataset(torch.utils.data.Dataset):
    def __init__(self, root_path, device = "cuda"):
        ...
        self.subset_size = len(self.file_list)
        self.device = device
        
        data_list = []
        for file_path in self.file_list:
            curr_data = np.load(root_path + file_path)
